...
std::shared_ptr<Scene> Assignment5::CreateScene() const
{
    ...
    glossy->SetDiffuse(glm::vec3(1.f, 1.f, 1.f));
    glossy->SetSpecular(glm::vec3(0.6f, 0.6f, 0.6f), 40.f);

    
    // Objects_sphere
    std::vector<std::shared_ptr<aiMaterial>> loadedMaterials1;
    std::vector<std::shared_ptr<MeshObject>> glossy_sphere = MeshLoader::LoadMesh("glossy.obj", &loadedMaterials1);
    for (size_t i = 0; i < glossy_sphere.size(); ++i) {
//        std::shared_ptr<Material> materialCopy = glossy->Clone();
//        materialCopy->LoadMaterialFromAssimp(loadedMaterials1[i]);
        glossy_sphere[i]->SetMaterial(glossy);
    }
    
    // Material_mirror
    std::shared_ptr<BlinnPhongMaterial> mirror = std::make_shared<BlinnPhongMaterial>();
//    glossy->SetDiffuse(glm::vec3(0.5f, 0.5f, 0.5f));
//    glossy->SetSpecular(glm::vec3(0.8f, 0.8f, 0.8f), 20.f);
    mirror->SetReflectivity(0.8f);
...
//        materialCopy->LoadMaterialFromAssimp(loadedMaterials2[i]);
    ...
    cubeSceneObject->AddMeshObject(glossy_sphere);
